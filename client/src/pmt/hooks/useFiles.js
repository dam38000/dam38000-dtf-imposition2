...
export function useFiles({ autoCrop, setErrorAlert, resetPlanche }) {
  ...
  const croppingRef = useRef(new Set()); // IDs en cours de crop
  const cropFile = useCallback(async (id, force = false) => {
    if (!force && croppingRef.current.has(id)) return; // déjà en cours
    croppingRef.current.add(id);
    try {
      console.log('[crop] Rognage de', id);
  ...
  const cropAll = useCallback(() => {
    setFiles(prev => { prev.forEach(f => cropFile(f.id, true)); return prev; });
  }, [cropFile]);
