...
export default function FinesseModal_Seri({ file, finesse, onClose, onCorrectFinesse, onExpandBordure, onSave, setFiles }) {
  ...
  const [customBgScale, setCustomBgScale] = useState(() => parseFloat(localStorage.getItem('seri_finesse_custom_bg_scale')) || 1);
  ...
  const [patchColors, setPatchColors] = useState(() => [
    localStorage.getItem('seri_finesse_patch0') || '#2563eb',
    localStorage.getItem('seri_finesse_patch1') || '#dc2626',
    localStorage.getItem('seri_finesse_patch2') || '#16a34a',
    localStorage.getItem('seri_finesse_patch3') || '#1a1a1a',
    localStorage.getItem('seri_finesse_patch4') || '#ff9800',
    localStorage.getItem('seri_finesse_patch5') || '#9c27b0',
  ]);

  const savePatchColor = (idx, color) => {
    setPatchColors(prev => { const next = [...prev]; next[idx] = color; return next; });
    localStorage.setItem(`seri_finesse_patch${idx}`, color);
    setBgColor(color);
  };
        ...
        <div className="flex flex-1 gap-3 min-h-0 relative">
              ...
              {customBgData && (
                <>
                  <input type="range" min="0.5" max="2" step="0.1" value={customBgScale}
                    onChange={e => { const v = parseFloat(e.target.value); setCustomBgScale(v); localStorage.setItem('seri_finesse_custom_bg_scale', String(v)); }}
              ...
              onChange={e => {
                ...
                reader.onload = () => {
                  ...
                  tmpImg.onload = () => {
                    ...
                    setBgMode('custom');
                    try {
                      localStorage.setItem('seri_finesse_custom_bg', dataUrl);
                      localStorage.setItem('seri_finesse_custom_bg_scale', '1');
          ...
          >
            <div style={{
              ...
              <div style={{ position: 'relative', width: `${(file.widthPx || 1000) * SCALE_REAL * zoomLevel}px`, height: `${(file.heightPx || 1000) * SCALE_REAL * zoomLevel}px`, flexShrink: 0 }}>
                ...
                />
                {/* Overlay finesses masqué pour SeriQuadri */
                false && (
                  <img
                    src={file.overlaySrc}
          ...
          >
            <div style={{
              ...
              {/* Panneau droit vide pour SeriQuadri */}
