...
export default function FinesseModal_DTF({ file, finesse, onClose, onCorrectFinesse, onExpandBordure, onSave }) {
  ...
  const [customBgData, setCustomBgData] = useState(() => localStorage.getItem('dtf_finesse_custom_bg') || null);
  const [customBgScale, setCustomBgScale] = useState(() => parseFloat(localStorage.getItem('dtf_finesse_custom_bg_scale')) || 1);
  ...
  const [patchColors, setPatchColors] = useState(() => [
    localStorage.getItem('dtf_finesse_patch0') || '#2563eb',
    localStorage.getItem('dtf_finesse_patch1') || '#dc2626',
    localStorage.getItem('dtf_finesse_patch2') || '#16a34a',
    localStorage.getItem('dtf_finesse_patch3') || '#1a1a1a',
    localStorage.getItem('dtf_finesse_patch4') || '#ff9800',
    localStorage.getItem('dtf_finesse_patch5') || '#9c27b0',
  ]);

  const savePatchColor = (idx, color) => {
    setPatchColors(prev => { const next = [...prev]; next[idx] = color; return next; });
    localStorage.setItem(`dtf_finesse_patch${idx}`, color);
    setBgColor(color);
  };
              ...
              {customBgData && (
                <>
                  <input type="range" min="0.5" max="2" step="0.1" value={customBgScale}
                    onChange={e => { const v = parseFloat(e.target.value); setCustomBgScale(v); localStorage.setItem('dtf_finesse_custom_bg_scale', String(v)); }}
            ...
            <input
              ...
              onChange={e => {
                ...
                reader.onload = () => {
                  ...
                  setCustomBgScale(1);
                  setBgMode('custom');
                  localStorage.setItem('dtf_finesse_custom_bg', dataUrl);
                  localStorage.setItem('dtf_finesse_custom_bg_scale', '1');
                };
